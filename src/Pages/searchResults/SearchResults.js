...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const storedSearchedPeople = JSON.parse(
      sessionStorage.getItem('searchPeopleResults')
    );
    ...
    <div className="p-5 md:px-12 grid grid-cols-1 md:grid-cols-3 gap-5">
      <div className="md:col-span-2">
        ...
        <h1 className="text-4xl font-bold">
          <span className="text-[#6B6B6B]">Results for</span> {searchedQuery}
        </h1>
        <div className="flex gap-3 items-center text-xs font-semibold my-5 cursor-pointer">
          <p
            onClick={() => setSelectedResult('stories')}
            className={`${selectedResult === 'stories' && 'underline'}`}
          >
            Stories
          </p>
          <p
            onClick={() => setSelectedResult('people')}
            className={`${selectedResult === 'people' && 'underline'}`}
          >
            People
          </p>
        </div>
