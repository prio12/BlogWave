...
//   console.log(currentUser?.followers);
  ...
  if (previousPage === "/profile") {
    ...
    content = visitProfile?.followers.map((follower) =>(
        // <div key={follower._id} className="flex items-center my-5 justify-between">
        //     <img className="h-16 w-16" src={follower.profilePic} alt="" />
        //     <p className="text-xs font-bold">{follower.name}</p>
        // </div>
    //     <div className="md:flex lg:flex items-center my-5 gap-5">
    //     <img src={follower?.profilePic} className="h-16 w-16" alt="" />
    //     <div className="my-3 md:my-0">
    //       <p className="text-xs font-bold">{follower?.name}</p>
    //       {follower.about && (
