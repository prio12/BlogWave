...
const Library = () => {
  ...
  if (bookmarks?.length) {
    content = bookmarks.map((blog) => (
      <Blogs blog={blog} key={blog._id}></Blogs>
    ));
  }

  if (!clapped || !clapped.length) {
    clappedContent = (
      <p className="text-xs text-center py-5">You have not clapped any post!</p>
    );
  }

  if (clapped && clapped.length) {
    clappedContent = clapped.map((blog) => (
      <Blogs blog={blog} key={blog._id}></Blogs>
    ));
  }
        ...
        <div>
          <div className="flex gap-5 items-center cursor-pointer my-5 ms-1">
            <p onClick={() => handleToggle('bookmarks')}>
              <small
                className={` ${activeContent === 'bookmarks' && 'underline'}`}
              >
                Bookmarks
              </small>
            </p>
            <p onClick={() => handleToggle('clapped')}>
              <small
                className={` ${activeContent === 'clapped' && 'underline'}`}
              >
                Clapped
              </small>
            </p>
          ...
          <div>
            {activeContent === 'bookmarks' && <div>{content}</div>}
            {activeContent === 'clapped' && <div>{clappedContent}</div>}
          </div>
        </div>
      </div>
